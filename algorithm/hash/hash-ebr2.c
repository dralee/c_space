...
/*
gcc -std=gnu11 -pthread -DTEST_MULTI -g hash-ebr.c -O0 -o hash-ebr.out
    valgrind --leak-check=full --show-leak-kinds=all ./hash-ebr.out
检测问题
...
void ebr_thread_unregister(void) {
    ...
    // Force free any remaining nodes (they should be safe by now)
    pthread_mutex_lock(&ebr_table_lock);
    RetireNode *rn = ebr_table[id].retire_list;
    extern void ebr_free_entry_resources(Entry *e);
    while (rn) {
        RetireNode *next = rn->next;
...
void ebr_free_entry_resources(Entry *e) {
    ...
    if (ebr_global_free_value) {
        ...
        if (v){
            ...
            atomic_store(&e->value, NULL); // 防止双重释放
        }
    }
}
    ...
    for (size_t probes = 0; probes < cap; ++probes) {
        ...
        if (slot_e->dist < new_e->dist) {
            ...
            if (atomic_compare_exchange_strong(&m->table[i], &expected, ptr_from_entry(new_e))) {
                ...
                continue;*/
                // 成功：槽位现在持有 new_e
                // 重用被置换的条目（只增加其距离）而不是创建副本
                Entry *to_reinsert = slot_e;
                to_reinsert->dist++;  // 增加探测距离
                new_e = to_reinsert;  // 继续插入这个条目
                // 注意：不退役 slot_e，因为我们正在重用它
                continue;
            } else {
                probes--;
                continue;
