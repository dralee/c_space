...
typedef struct Entry {
    void *key;
    _Atomic(void *) value;
    _Atomic(int) dist; // probe distance - atomic to safely read during displacement
} Entry;
...
void ebr_thread_unregister(void) {
    ...
    // try a few times to clear the retire list
    for (int attempt = 0; attempt < 10; ++attempt) {
        if (ebr_table[id].retire_list == NULL) break;
        atomic_fetch_add(&global_epoch, 1);
        ebr_scan();
        usleep(5000); // Sleep longer
    }

    // DON'T force free remaining nodes - let other threads clean them up
    // This is safer in a multi-threaded environment
    
    pthread_mutex_lock(&ebr_table_lock);
    ebr_table[id].in_use = 0;
    ebr_table[id].tid = 0;
    // Leave retire_list intact - other threads may still clean it up
    pthread_mutex_unlock(&ebr_table_lock);
    ebr_tid = -1;
}
...
void lfmap_destroy(LFMap *m) {
    ...
    // Advance epoch multiple times and scan to try to reclaim as much as possible
    for (int i = 0; i < 5; i++) {
        atomic_fetch_add(&global_epoch, 1);
        ebr_scan();
        usleep(1000);
    ...
    // free live entries in table
    // NOTE: We only free entries that are STILL in the table
    // Don't free their key/value as those might have been freed already
    for (size_t i = 0; i < m->capacity; ++i) {
        uintptr_t p = atomic_load(&m->table[i]);
        if (p != 0 && p != TOMBSTONE) {
            Entry *e = entry_from_ptr(p);
            // Free the entry itself, but let EBR handle key/value cleanup
            // Actually, if the entry is still in the table, it hasn't been retired yet
            // so we should free its key/value
    ...
    // final cleanup attempt for any remaining retired entries
    for (int i = 0; i < 3; i++) {
        atomic_fetch_add(&global_epoch, 1);
        ebr_scan();
    }
...
int lfmap_insert(LFMap *m, void *key, void *value) {
    ...
    for (size_t probes = 0; probes < cap; ++probes) {
        ...
        if (m->key_eq(slot_e->key, key)) {
            ...
            // free new entry's key and the entry itself (we don't need it)
        ...
        // Read distance atomically to avoid race with concurrent displacement
        int slot_dist = atomic_load(&slot_e->dist);
        int new_dist = atomic_load(&new_e->dist);

        // Robin Hood: if current slot's dist < new's dist, swap (steal)
        if (slot_dist < new_dist) {
            uintptr_t expected = cur;
            if (atomic_compare_exchange_strong(&m->table[i], &expected, ptr_from_entry(new_e))) {
                // success: slot now holds new_e
                // Simply reuse slot_e by incrementing its distance
                // DON'T retire slot_e - we're going to keep using it
                atomic_store(&slot_e->dist, slot_dist + 1);
                new_e = slot_e;
                continue;
            } else {
                // CAS failed, cur changed, retry this slot
                goto retry_slot;
            }
        }

        // otherwise advance
        atomic_store(&new_e->dist, new_dist + 1);
        continue;
    }
...
void *lfmap_get(LFMap *m, const void *key) {
    ...
    for (size_t probes = 0; probes < cap; ++probes) {
        ...
        int slot_dist = atomic_load(&slot_e->dist);
        if (dist > slot_dist) { ebr_exit(); return NULL; }
        dist++;
    }
    ebr_exit();
    return NULL;
    ...
    for (size_t probes = 0; probes < cap; ++probes) {
        ...
        if (m->key_eq(slot_e->key, key)) {
            ...
            if (atomic_compare_exchange_strong(&m->table[i], &expected, TOMBSTONE)) {
                ...
                // CAS failed - entry might have been deleted or replaced
                // Reload and retry this position
                probes--;
                continue;
            }
        }
        ...
        int slot_dist = atomic_load(&slot_e->dist);
        if (dist > slot_dist) { ebr_exit(); return 0; }
        dist++;
    }
    ebr_exit();
    return 0;
...
void *worker(void *arg) {
    ...
    for (int i = 0; i < OPS_PER_THREAD; ++i) {
        ...
        *v = k;
        lfmap_insert(m, ks, v);
        // Map takes ownership of ks and v on success
        // On update (unlikely with unique keys), map frees old value and our key

        // occasionally lookup
        if ((i & 7) == 0) {
            snprintf(buf, sizeof(buf), "key-%d", k);
            void *r = lfmap_get(m, buf);
            (void)r;
        }
        
        // occasionally delete our own keys
        if ((i & 31) == 0 && i > 0) {
            int del_k = id * OPS_PER_THREAD + (i - 1);
            snprintf(buf, sizeof(buf), "key-%d", del_k);
            lfmap_delete(m, buf);
        }
    }
